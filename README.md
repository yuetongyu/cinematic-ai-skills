...
- 动作四宫格与分镜四宫格是不同产物；每个四宫格固定输出为一张 16:9、2×2 的完整图片，不拆成四张图。
...
```text
使用 $craft-cinematic-action-design，把 B003 设计成四个有因果的动作相位，输出一张 16:9、2×2 的动作四宫格提示词。
```
...
| 类型 | 单张 16:9 图里的四格 | 解决什么 |
|---|---|---|
| 动作四宫格 | 同一动作的意图/预备、发动、接触、后果 | 重心、轨迹、接触、受力和 VFX 相位 |
| 分镜四宫格 | 四个连续电影镜头 | 观众位置、轴线、景别、剪辑和信息顺序 |

四宫格固定为一张横向 16:9 画布，内部 2×2 等尺寸面板，阅读顺序为左上、右上、左下、右下。四格共用身份、场景和连续性，只输出一条整图生成提示词；镜号、箭头和时间码建议后期排版。最终逐镜生产是后续 `SHOT_PACKET` 的职责，不属于四宫格交付。
